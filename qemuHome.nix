let
  ...
  path_windows_iso = "/home/auros/Win10_22H2_Korean_x64.iso";
...
in
{
  home = {
    shellAliases = {
      ...
      windows10_install="qemu-img create ${path_windows_raw} 70G | sudo qemu-system-x86_64 -m 6G -cpu host -smp sockets=1,cores=4,threads=4 -cdrom ${path_windows_iso} -drive file=${path_windows_raw},format=raw -enable-kvm -nic mac=${mac_alloc} -rtc base=2023-05-29T09:30:11";
      windows10_wo_network="sudo qemu-system-x86_64 -m 6G -cpu host -smp sockets=1,cores=4,threads=4 -drive file=${path_windows_raw},format=raw -enable-kvm -nic none";
      windows10_mac_fixed="sudo qemu-system-x86_64 -m 6G -cpu host -smp sockets=1,cores=4,threads=4 -drive file=${path_windows_raw},format=raw -enable-kvm -device e1000,mac=${mac_alloc}";
      ...
      windows10="sudo qemu-system-x86_64 -m 6G -cpu host -smp sockets=1,cores=4,threads=4 -drive file=${path_windows_raw},format=raw -enable-kvm";
